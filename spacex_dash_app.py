...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        success_count = filtered_df[filtered_df['class'] == 1]['Launch Site'].count()
        failure_count = filtered_df[filtered_df['class'] == 0]['Launch Site'].count()
        fig = px.pie(names=['Success', 'Failure'], values=[success_count, failure_count],
                     title=f"Total Launches Outcome for {entered_site}")
    return fig
